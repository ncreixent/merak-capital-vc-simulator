...
def render_time_analysis_tab(scenario):
    ...
    st.markdown('<h2 class="section-header">📊 Portfolio Performance Overview</h2>', unsafe_allow_html=True)
    ...
    # 3. COMPREHENSIVE STATISTICS (Collapsible)
    st.markdown('<h2 class="section-header">📋 Comprehensive Statistics</h2>', unsafe_allow_html=True)
    
    with st.expander("📊 Company Outcome Statistics", expanded=False):
...
def render_portfolio_composition_widget(scenario):
    ...
    fig.add_trace(go.Bar(
        ...
        y=avg_counts['count'],
        marker_color=['#1e3a8a', '#3b82f6', '#10b981'],  # Professional color scheme
