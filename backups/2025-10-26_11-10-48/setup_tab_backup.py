...
def render_scenario_list():
    ...
    for i, row in enumerate(scenario_data):
        ...
        with col9:
            if st.button("📋", key=f"view_summary_{i}", help="View configuration summary"):
                st.session_state.selected_scenario_for_summary = row['Scenario Name']
                st.rerun()
        with col10:
            if st.button("🗑️", key=f"delete_scenario_{i}", help="Delete scenario"):
        ...
        with st.spinner("Importing scenario..."):
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if scenario['name'] in st.session_state.scenarios:
                    st.warning(f"⚠️ A scenario named '{scenario['name']}' already exists.")
