...
def calculate_average_waterfall(scenarios):
    ...
    scenarios_with_waterfall = [s for s in scenarios.values() if s.get('waterfall_log') is not None and not s['waterfall_log'].empty]
    
    if not scenarios_with_waterfall:
        return None
    
    # Combine all waterfall data from all scenarios
    all_waterfall_data = []
    for scenario in scenarios_with_waterfall:
